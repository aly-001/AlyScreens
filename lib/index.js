...
class DolphinSR {
  ...
  summary() {
    const s = this._getCardsSchedule();
    // get new cards count using getNewCards()
    const newCount = this.getNewCards().length;
    const matureCount = this.getMatureCards().length;
  ...
  getLearningCards() {
    ...
    const newCardIds = new Set(newCards.map(card => card.master));


    return s.learning
      .map(cardId => this._getCard(cardId))
      .filter(card => !newCardIds.has(card.master));
  }
  ...
  getDueCards(){
    ...
    Object.keys(this._state.cardStates).forEach((cardId) => {
      const cardState = this._state.cardStates[cardId];
      if (cardState.interval >= 21 && cardState.interval <= 30) {
        dueCards.push(this._getCard(cardId));
      }
    });
    return dueCards;
  }
